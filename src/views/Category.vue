<template>
    ...
    <h1 class="mt-2 text-5xl font-extrabold text-black text-center mt-9">ALL-CATEGORY</h1>
    <div class="grid grid-cols-2 gap-2 m-5">
        <div v-for="category in getCategory.data.splice(0,8)" >
      <div>
  ...
  <div class="flex flex-col overflow-hidden bg-white sm:flex-row md:h-80">
    <div class="flex w-full flex-col p-4 sm:w-1/2 sm:p-8 lg:w-3/5">
      <h2 class="text-xl font-bold text-gray-900 md:text-2xl lg:text-4xl">{{ category.slug }}</h2>
    ...
    <div class="order-first ml-auto h-48 w-full bg-gray-700 sm:order-none sm:h-auto sm:w-1/2 lg:w-2/5">
      <img class="h-full w-full object-cover" :src="category.banner" loading="lazy" />
    </div>
  </div>
</div>
